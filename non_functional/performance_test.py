...
DATA_PATH = os.path.join(ROOT_DIR, "data", "non_functional", "performance_data.csv")
RESULT_PATH = os.path.join(ROOT_DIR, "results", "non_functional", "performance_results.csv")
...
def run():
    ...
    try:
        for row in rows:
            ...
            try:
                # TODO: Measure page load time and compare to expected threshold.
                driver.get(BASE_URL)

                actual = "TODO"
                if expected and not text_equals(expected, actual):
                    status = "FAIL"
